...
import { getRequestsByInvestorId } from '@/lib/firestore';
...
const InvestorNotificationsPage = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        setLoading(true);
        const investorRequests = await getRequestsByInvestorId(currentUser.uid);
        setRequests(investorRequests.map(convertTimestamps));
      } catch (error) {
            ...
            <Card key={request.id}>
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle>Investment Request to {request.startupName || 'Startup'}</CardTitle>
                    <CardDescription>
